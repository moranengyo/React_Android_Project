...
public interface UsageRepository extends JpaRepository<UsageEntity, Long> {
    ...
    @Query(value = "select * from item_usage where date_format(usage_time, :dateForm) = date_format(now(), :dateForm) ", nativeQuery = true)
    ...
    @Query(value =
            "select i_info.item_id as itemId, " +
                    ...
                    "ifnull(sum(i_info.total_num), 0) as totalNum, " +
